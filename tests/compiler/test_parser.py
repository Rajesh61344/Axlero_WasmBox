...
def test_parse_valid_simple(simple_plugin_source):
    ...
    entrypoint_info = next(
        (d for d in result.diagnostics if "Entrypoint" in d.message),
        None,
    )
...
"""
    ...
    assert (
        "expected ':'" in exc_info.value.message
        or "invalid syntax" in exc_info.value.message
    )
...
def test_parse_too_many_ast_nodes():
    limits = CompilationLimits(max_ast_nodes=10)
    parser = PluginParser(limits=limits)
...
"""
    ...
    assert "AST node count" in exc_info.value.message
    assert "exceeds maximum allowed" in exc_info.value.message
...
def test_parse_entrypoint_detected(simple_plugin_source):
    ...
    has_entrypoint = any(
        "Entrypoint function 'main' found" in d.message
...
def test_parse_multiple_functions(calculator_plugin_source):
    ...
    diag = next(
        (d for d in result.diagnostics if "Parsed" in d.message),
        None,
    )
