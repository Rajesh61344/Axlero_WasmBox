...
def test_analyze_host_functions(host_function_source, default_config):
    ...
    analyzer = StaticAnalyzer()
    result = analyzer.analyze(tree, host_function_source, default_config)
...
"""
    ...
    assert set(result.global_variables) == {"counter", "API_KEY"}

def test_analyze_loops(infinite_loop_source, default_config):
    tree = get_ast(infinite_loop_source)
    analyzer = StaticAnalyzer()
    result = analyzer.analyze(tree, infinite_loop_source, default_config)
...
"""
    ...
    deps = {d.name: d.category for d in result.dependencies}
